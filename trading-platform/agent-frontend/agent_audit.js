...
function analyzeFile(filePath) {
    ...
    while ((match = jsxTextRegex.exec(content)) !== null) {
        ...
        if (text && text.length > 1 && !/^[0-9\W_]+$/.test(text) && !text.includes('i18n')) {
    ...
    // 3. Find string attributes (labels, titles, placeholders, messages)
    const attrRegex = /(?:placeholder|title|label|alt|message|text|name)\s*=\s*['"`]([^'"`{}]*)['"`]/g;
    ...
    const toastRegex = /toast\.(?:error|success)\s*\(\s*['"`]([^'"`#{}]+)['"`]/g;
...
fs.writeFileSync('agent_audit_results.json', JSON.stringify(results, null, 4));
